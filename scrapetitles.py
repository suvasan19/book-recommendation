...
try:
    print("Read from Books Pickle")
    booksdf = pd.read_pickle('goodreads_book_works.pkl')
except (OSError, IOError) as e:
    print("Read from Books JSON")
    booksdf = pd.read_json("goodreads_book_works.json",lines=True)
    cols = ["best_book_id", "original_title", 'default_chaptering_book_id']
    booksdf = booksdf[cols]
    booksdf.to_pickle('goodreads_book_works.pkl')
...
df.to_pickle('goodreads_reviews_spoiler_processed_scraped.pkl')
